...
{ 
  ...
  if (win(tokens1))
  {
    println("Conglats. You win against a dumb computer. Good job. not.");
    exit();
  } else if (win(tokens2))
  {
    println("Sorry. not. You lose. I guess you must bow down to your robotic overlords. Or maybe not.");
    exit();
  }
}
...
void mouseClicked()
{
  ...
  if (turn > 0)
  {
    move1(click(mouseX, mouseY), roll);
  } else if (turn < 0)
  {
    int i = 0;
    while (turn < 0)
...
{
  //Can the player move?
  if (piece != -2 && find(piece, tokens1) != -1)
  {
    if (amount == 0)
    {
      //Dummy turn
